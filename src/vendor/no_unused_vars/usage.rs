// Synced from oxc @ 14533a3dc118bea73e755426aaf35f71dbe81eb8: crates/oxc_linter/src/rules/eslint/no_unused_vars/usage.rs
...
impl<'a> Symbol<'_, 'a> {
        ...
        for reference in self.references() {
            ...
            if reference.is_type() {
                ...
                if options.report_vars_only_used_as_types
                    && !self
                        .flags()
                        .intersects(SymbolFlags::TypeImport.union(SymbolFlags::Import))
        ...
        for parent in self.nodes().ancestor_kinds(reference.node_id()) {
            match parent {
                ...
                AstKind::AssignmentExpression(assignment) => {
                    return options
                        .is_ignored_assignment_target(self, &assignment.left)
                        .is_some();
                }
    ...
    fn is_type_self_usage(&self, reference: &Reference) -> bool {
        for parent in self
            .iter_relevant_parents_of(reference.node_id())
            .map(AstNode::kind)
        {
            match parent {
    ...
    fn return_type_predicate_references_symbol(
        ...
        let Some(TSTypeAnnotation {
            type_annotation: TSType::TSTypePredicate(predicate),
            ..
        }) = return_type
        else {
            return false;
    ...
    fn is_self_call_simple(&self, reference: &Reference) -> bool {
        ...
        if redeclarations.is_empty() {
            self.declaration()
                .span()
                .contains_inclusive(self.get_ref_span(reference))
        } else {
    ...
    pub fn has_reference_used_as_type_query(&self) -> bool {
        self.references()
            .any(|reference| self.reference_contains_type_query(reference))
    }
    ...
    fn get_parent_variable_scope(&self, scope_id: ScopeId) -> ScopeId {
        let mut last = None;
        for scope_id in self.scoping().scope_ancestors(scope_id) {
            last = Some(scope_id);
            if self.scoping().scope_flags(scope_id).is_var() {
                return scope_id;
            }
        }
        last.expect("scope iterator will always contain at least one element")
    }
}
