// Synced from oxc @ 14533a3dc118bea73e755426aaf35f71dbe81eb8: crates/oxc_linter/src/rules/eslint/no_unused_vars/symbol.rs
...
use oxc_syntax::module_record::{ExportLocalName, ModuleRecord};
...
impl<'s, 'a> Symbol<'s, 'a> {
    ...
    #[inline]
    pub fn semantic(&self) -> &'s Semantic<'a> {
        self.semantic
    }

    #[inline]
    pub fn module_record(&self) -> &'s ModuleRecord<'a> {
        self.module_record
    }
    ...
    pub fn iter_relevant_parents_of(
        ...
        self.nodes()
            .ancestors(node_id)
            .filter(|n| Self::is_relevant_kind(n.kind()))
    }
...
impl<'a> Symbol<'_, 'a> {
    ...
    pub fn collect_exported_local_names(module_record: &ModuleRecord<'a>) -> FxHashSet<&'a str> {
...
impl<'a> PartialEq<IdentifierReference<'a>> for Symbol<'_, 'a> {
    fn eq(&self, other: &IdentifierReference<'a>) -> bool {
        ...
        reference
            .symbol_id()
            .is_some_and(|symbol_id| self.id == symbol_id)
    }
}
