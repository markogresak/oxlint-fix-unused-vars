// Synced from oxc @ 14533a3dc118bea73e755426aaf35f71dbe81eb8: crates/oxc_linter/src/rules/eslint/no_unused_vars/allowed.rs
...
use super::binding_pattern::{BindingContext, HasAnyUsedBinding};
...
impl NoUnusedVars {
    ...
    pub(super) fn is_allowed_argument<'a>(
        ...
        let Some(params) = symbol
            .nodes()
            .parent_kind(argument.node_id())
            .as_formal_parameters()
        else {
            debug_assert!(
                false,
                "FormalParameter should always have a parent FormalParameters"
            );
            return false;
        };

        if let FunctionParameterKind::Normal(param) = argument {
            if Self::is_allowed_param_because_of_method(semantic, param) {
                return true;
            }
        }
    ...
    fn is_allowed_param_because_of_method<'a>(
        ...
        match maybe_method_or_fn {
            ...
            | AstKind::ObjectProperty(ObjectProperty {
